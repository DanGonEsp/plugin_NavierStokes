...
#ifndef __H__UG__LIB_DISC__SPATIAL_DISC__ELEM_DISC__DENSITY_DRIVEN_FLOW__FV1__CONSISTENT_GRAVITY_MULTIPHASE__
#define __H__UG__LIB_DISC__SPATIAL_DISC__ELEM_DISC__DENSITY_DRIVEN_FLOW__FV1__CONSISTENT_GRAVITY_MULTIPHASE__
...
///	computation of the primary function for the consistent gravity at corners of a triangle
	...
	)
	{
		...
		vConsGravity[0][_X_] = 0.0;
		vConsGravity[1][_X_] = LocalGravity[_X_]*(vDensity[0] + vDensity[1])*0.5;
        vConsGravity[2][_X_] = 0.0;

		vConsGravity[0][_Y_] = 0.0; 
        vConsGravity[1][_Y_] = 0.0;
...
///	computation of the consistent gravity at a given point
	...
	)
	{
		...
		//	special method for triangles and tetrahedra, currently only in the full dimension
            ...
            {
                const size_t sh = d+1;
                LocalGravity[d] = vConsGravity[sh][dim-1];
            }
...
///	computation of the extended version of the corner consistent gravity for simplices (only in full dimension!)
	...
	)
	{
        ...
        MathVector<refDim> LocalPoint; VecSet(LocalPoint,0.0);
		MathVector<dim> ShiftedGlobalPoint;
		MathMatrix<dim,refDim> JT;
		static ReferenceMapping<refElem, dim> Mapping;
		Mapping.update (vCorners);
		Mapping.jacobian_transposed_inverse (JT, LocalPoint); // the 2. argument is dummy

		for (size_t sh = 1; sh < dim+1; sh++)
		{
			VecSubtract (ShiftedGlobalPoint, vCorners[sh], vCorners[0]);
			...
			for(size_t j = 0; j < dim-1; ++j)
			{
				...
				vConsGravity[sh][j] = Diff * PhysicalGravity[dim-1] * gradient;
			}
			vConsGravity[sh][dim-1] = Diff * PhysicalGravity[dim-1] * (vDensity[sh] + DensityIP)*0.5;
		}
	}
};
...
#endif /* __H__UG__LIB_DISC__SPATIAL_DISC__ELEM_DISC__DENSITY_DRIVEN_FLOW__FV1__CONSISTENT_GRAVITY_MULTIPHASE__ */
