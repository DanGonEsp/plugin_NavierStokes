...
#ifndef __H__UG__PLUGINS__NAVIER_STOKES__INCOMPRESSIBLE__FV1__PRESSURE_GRADIENT__
#define __H__UG__PLUGINS__NAVIER_STOKES__INCOMPRESSIBLE__FV1__PRESSURE_GRADIENT__
...
class NodalPressureGradient
: 	public StdUserData<NodalPressureGradient<TGridFunction>, MathVector<TGridFunction::dim>, TGridFunction::dim>,
    ...
    typedef typename Grid::VertexAttachmentAccessor<ADimVector> aVertexVector;
	...
	//	pressure attachment accessor (interpolated pressure grad in vertices)
    ADimVector m_aDP;
    aVertexVector m_Dp;
	...
	void set_source(const char* fctName)
	{
		set_source(LuaUserDataFactory<MathVector<dim>, dim>::create(fctName));
  		...
  		public:
	/// constructor
        NodalPressureGradient(SmartPtr<ApproximationSpace<domain_type> > approxSpace,SmartPtr<TGridFunction> spGridFct){
		m_u = spGridFct;
		...
		set_source(0.0);
		grid.template attach_to<Vertex>(m_aDP);
		grid.template attach_to<Vertex>(m_aVol);
		m_Dp.access(grid,m_aDP);
		m_vol.access(grid,m_aVol);
		// set all values to zero
		SetAttachmentValues(m_vol, m_u->template begin<Vertex>(), m_u->template end<Vertex>(), 0);
		SetAttachmentValues(m_Dp, m_u->template begin<Vertex>(), m_u->template end<Vertex>(), 0);

		this->update();
	}

	virtual ~NodalPressureGradient(){};
...
//		position_accessor_type aaPos = m_u->domain()->position_accessor();
	...
	void update(){
		//	get domain
        printf("Updating pressure gradient... \n");
		domain_type& domain = *m_u->domain().get();
		//	create Multiindex
		...
		for(int si = 0; si < domain.subset_handler()->num_subsets(); ++si){
			...
			for(  ;iter !=iterEnd; ++iter)
			{
				...
				for(size_t i = 0; i < numVertices; ++i){
                    ...
                    m_u->dof_indices(elem->vertex(i), _P_, multInd);
                    //    read value of index from vector
                    number pVal = DoFRef(*m_u,multInd[0]);
                    
                    
                    for(size_t d = 0; d < dim; ++d){

                        m_Dp[vVrt[i]][d] += pVal * geo.scv(i).global_grad(i)[d] * scvVol;
                    }
				}
			}
		}
...
#endif /* __H__UG__PLUGINS__NAVIER_STOKES__INCOMPRESSIBLE__FV1__PRESSURE_GRADIENT__ */
